...
function UI.init()
	...
	for start = 0, 2 do
		...
		for y = 0, 2 do
			for x = 0, 2 do
				button.quads[start][y * 3 + x] = love.graphics.newQuad(start * button.sw * 2 + x * button.sw,
...
function UI.button(label, x, y, w, h)
	...
	Util.print(label, x + (w - font:getWidth(label) - 7) / 2, y + (h - 12) / 2)
...
function UI.area(start, x, y, w, h)
	...
	love.graphics.draw(button.texture, button.quads[start][6], x, y + h - button.sh * 2, 0, 1, 1)
	love.graphics.draw(button.texture, button.quads[start][6], x + w - button.sw, y + h - button.sh * 2, 0, -1, 1)
	love.graphics.draw(button.texture, button.quads[start][7], x + button.sw, y + h - button.sh * 2, 0, (w - button.sw * 3) / button.sw, 1)
